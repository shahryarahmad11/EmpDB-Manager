...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($fullname) || empty($email) || empty($password) || empty($city) || empty($university)) {
        ...
        if ($chk && sqlsrv_fetch_array($chk, SQLSRV_FETCH_ASSOC)) {
            ...
            write_activity_log_as($fullname, 'SIGNUP_FAILED', "Signup attempt failed — duplicate email: $email");
        } else {
            ...
            if ($ok) {
                write_activity_log_as($fullname, 'SIGNUP', "New account registered with email=$email, City=$city, University=$university");
...
<div class="card">
    ...
    <form method="POST" autocomplete="off">
        <!-- Hidden fake fields to stop browser autofill -->
